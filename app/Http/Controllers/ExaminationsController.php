...
class ExaminationsController extends Controller
{
    ...
    public function exam_schedule(Request $request)
    {
        ...
        $result = array();
        if (!empty($request->get('exam_id')) && !empty($request->get('class_id')) && !empty($request->get('subject_id')) && !empty($request->get('college_id'))) {
            $getSubject = ClassAndStudentsModel::MySubject($request->get('class_id'), $request->get('subject_id'), $request->get('college_id'));
            // dd($getSubject);
            foreach ($getSubject as $value) {
                ...
                $dataS['college_name'] = $value->college_name;
                $dataS['session_name'] = $value->session_name;
                $dataS['college_id'] = $value->college_id;
                // dd($request->all());
                $ExamSchedule = ExamScheduleModel::getRecordSingle($request->exam_id, $request->class_id, $request->subject_id, $request->college_id, $request->session_id);
                // dd($request->class_id);
                // dd($ExamSchedule);
                if (!empty($ExamSchedule)) {
                    $dataS['college_id'] = $ExamSchedule->college_id;
                    $dataS['subject_id'] = $ExamSchedule->subject_id;
                    $dataS['session_id'] = $ExamSchedule->session_id;
    ...
    public function exam_schedule_insert(Request $request)
    {
        ...
        ExamScheduleModel::deleteRecord($request->exam_id, $request->class_id, $request->subject_id, $request->college_id, $request->session_id);
        if (!empty($request->schedule)) {
            foreach ($request->schedule as $schedule) {
                if (!empty($schedule['exam_date']) && !empty($schedule['start_time']) && !empty($schedule['end_time']) &&  !empty($schedule['full_marks']) && !empty($schedule['passing_mark'])) {
                    ...
                    $exam->college_id = $schedule['college_id'];
                    $exam->session_id = $schedule['session_id'];
                    $exam->exam_date = $schedule['exam_date'];
                    $exam->start_time = $schedule['start_time'];
                    $exam->end_time = $schedule['end_time'];
                    $exam->room_number = StudentClass::getSingleClass($request->class_id)->room;
    ...
    public function marks_register(Request $request)
    {
        ...
        if (!empty($request->get('exam_id')) && !empty($request->get('class_id')) && !empty($request->get('subject_id')) && !empty($request->get('college_id'))) {
            $data['getSubject'] = ExamScheduleModel::getSubject($request->get('exam_id'), $request->get('class_id'), $request->get('subject_id'), $request->get('college_id'), $request->get('session_id'));
    ...
    public function submit_marks_register(Request $request)
    {
        ...
        if (!empty($request->mark)) {
            foreach ($request->mark as $mark) {
                ...
                if ($full_marks >= $total_mark) {
                    ...
                    $save->college_id = $request->college_id;
                    $save->session_id = $request->session_id;
        ...
        if ($valaition == 1) {
            return redirect()->back()->with('error', "Your total mark greater than full mark");
        } else {
            return redirect()->back()->with('success', "Mark Register successfully saved");
        }
    }
